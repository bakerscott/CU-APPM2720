...
Team apllication Exercise:
```{r}
tan1 = read.csv("~/Desktop/Tanganyika.csv") %>% select(-X, -X.1, -X.2, -X.3, -X.4, -X.5) %>% select(Year.AD, BSi..normalized.)
bsi = rename(tan1, Year = Year.AD)

tan2 = read.csv("~/Desktop/Tanganyika.csv") %>% select(-X, -X.1, -X.2, -X.3, -X.4, -X.5) %>% select(Year.AD.1, Charcoal..normalized.)

char = rename(tan2, Year = Year.AD.1)

tan3 = read.csv("~/Desktop/Tanganyika.csv") %>% select(-X, -X.1, -X.2, -X.3, -X.4, -X.5) %>% select(Year, TEX86)
