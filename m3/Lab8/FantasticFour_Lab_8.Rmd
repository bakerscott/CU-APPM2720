...
mp <- ggplot() +  mapWorld 
mp <- mp + geom_point(data = forMap, mapping = aes(x=LONGITUDE,y=LATITUDE,color = PRIMARY_MAGNITUDE),size = 0.3)
mp

qmap(location = "hawaii",zoom = 3) + geom_point(data = forMap, mapping = aes(x=LONGITUDE,y=LATITUDE,color = PRIMARY_MAGNITUDE),size = 0.5)

qmap(location = "honolulu",zoom = 7) + geom_point(data = forMap, mapping = aes(x=LONGITUDE,y=LATITUDE,color = PRIMARY_MAGNITUDE),size = 3)
```
...
damage_tsunami = tsunami %>%
  ...
  ggplot(damage_tsunami, mapping = aes(x = YEAR , y = damage)) + geom_jitter() + geom_smooth(se = FALSE) + ylab("All Damage (Million Dollars)")
cor(damange_tsunami$YEAR, damange_tsunami$damage)
```
As we can see from the plot and calculated correlation, there is no relationship between the time of year a tsunami occurs and how much damage is inflicted but the reason why trend line is concave up after 1975 may be the increase of population and construction in the modern world.
...
## Team Plot
```{r, echo=FALSE, message= FALSE}
Malawi_team = normMalawiBSi %>% select(-BSi) %>% filter(Year >= 1300)
...
M_T = full_join(Malawi_team, Tanganyika_team)%>% 
  rename(MALAWI_standardized_BSi = normMalawiBsi, TANGANYIKA_standardized_charcoal = Charcoal_standard)
...
teamdata1 = M_T %>% gather(MALAWI_standardized_BSi, TANGANYIKA_standardized_charcoal, key = "Type", value = "Measurement" )

teamdata2 = V_C %>% gather(CHALLA_standardized_thick, VOI_standardized_rainfall, key = "Type", value = "Measurement" )

p <- ggplot()  + geom_smooth(teamdata1, mapping = aes(x= Year, y = Measurement, color = Type), se = FALSE ) +
 geom_smooth(teamdata2, mapping = aes(x= Year, y = Measurement, color = Type), se = FALSE )
p + ggtitle("East Africa Lake Data Visualization")
```

Our teamplot plots one of the standardized values from each data set vs. different year as four trendlines from 1300 to 2000.
...
* Li: Created individual plot of the relationship between the time of year a tsunami occurs and the damage is inflicted. Found the problem that we need to use github link to read data rather than using the local file on each of our computer and the problem was solved by Scott. Also, tidied the Tanganyika dataset and created the team plot of East Africa Lake and Rainfall Data.
