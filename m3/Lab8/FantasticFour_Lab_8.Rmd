...
damange_tsunami = tsunami %>%
  ...
  ggplot(warning_tsunami, mapping = aes(x = YEAR , y = damage)) + geom_jitter() + geom_smooth(se = FALSE) + ylab("All Damage (Million Dollars)")
cor(damange_tsunami$YEAR, damange_tsunami$damage)
```
As we can see from the plot and calculated correlation, there is no strong relationship between the time of year a tsunami occurs and how much damage is inflicted but the reason why trend line is concave up after 1975 may be the increse of population and construction in the modern world.
...
* Lexie: Created individual plots of the rain data and tsunami data with descriptions. Also helped contribute to the overall question and importance along with who is hiring us. Also created some of the individual questions and helped tidy data.
