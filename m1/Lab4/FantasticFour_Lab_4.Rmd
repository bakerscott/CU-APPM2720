...
```{r}
ggplot(data= babies,mapping= aes(x= mpregwt , y= gestation))+ geom_jitter() +facet_wrap(~smoke, labeller= labeller(smoke=newLabel))
```
...
I made two plots, one is showing the how mothers' pre-pregnancy weights affcted premature by using gemo_point to show the relationship and facet_grid to divide into smokers and non smokers. Another plot is showing how parity affcted premature by using gemo_violin to show mean of gestational age and facet_grid to divide into smokers and non smokers.
