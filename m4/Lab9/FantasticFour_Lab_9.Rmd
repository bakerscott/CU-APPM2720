...
batting <- select(batting,playerID,nameFirst,nameLast,birthDate,birthMonth,debut,finalGame,G,AB,R,HR,RBI,SB,SO,birthCity,birthState,birthCountry)
...
ggmap(get_map(location = 'USA',zoom = 3)) + geom_point(data=batting$geo,mapping=aes(x=lon,y=lat),color='purple') 
```
...
### 15.5.1.2:
```{r}
#mutate(gss_cat,rincome = fct_collapse(rincome, 'NA' = c("Don't know","Refused","Not applicable","No answer")),'known' = c("Lt $1000", str_c("$", c("1000", "3000", "4000"), " to ", c("2999", "3999", "4999")),str_c("$", c("5000", "6000", "7000", "8000")," to ", c("5999", "6999", "7999", "9999")),str_c("$", c("10000","15000","2000","25000"),"-",c("14999","19999","24999","more")))) 

```
### 16.4.5.3:
```{r}
dates <- c(20150101,20150201,20150301,20150401,20150501,20150601,20150701,20150801,20150901,20151001,20151101,20151201)
ymd(dates)
tday <- today()
tday
